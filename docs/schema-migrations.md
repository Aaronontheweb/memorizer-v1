...
- All migration scripts are stored in `Memorizer/migrations/` and named with an incrementing version prefix, e.g., `001_init.sql`, `002_add_column.sql`.
...
1. Create a new SQL file in `Memorizer/migrations/` with the next version number, e.g., `002_add_new_table.sql`.
...
- Run the integration tests: `dotnet test Memorizer.IntegrationTests`
